...
def on_message(client, userdata, msg):
    ...
    if msg.topic.startswith("solarflow-hub") and "control" in msg.topic and msg.payload:
        ...
        match parameter:
            case "sunriseOffset":
                (
                    log.info(f"Updating SUNRISE_OFFSET to {int(value)} minutes")
                    if SUNRISE_OFFSET != int(value)
                    else None
                )
                SUNRISE_OFFSET = int(value)
            case "sunsetOffset":
                (
                    log.info(f"Updating SUNSET_OFFSET to {int(value)} minutes")
                    if SUNSET_OFFSET != int(value)
                    else None
                )
                SUNSET_OFFSET = int(value)
            case "minChargePower":
                (
                    log.info(f"Updating MIN_CHARGE_POWER to {int(value)}W")
                    if MIN_CHARGE_POWER != int(value)
                    else None
                )
                MIN_CHARGE_POWER = int(value)
            case "maxDischargePower":
                (
                    log.info(f"Updating MAX_DISCHARGE_POWER to {int(value)}W")
                    if MAX_DISCHARGE_POWER != int(value)
                    else None
                )
                MAX_DISCHARGE_POWER = int(value)
            case "controlBypass":
            ...
            case "dischargeDuringDaytime":
                (
                    log.info(f"Updating DISCHARGE_DURING_DAYTIME to {str2bool(value)}")
                    if DISCHARGE_DURING_DAYTIME != str2bool(value)
                    else None
                )
                DISCHARGE_DURING_DAYTIME = str2bool(value)
            case "batteryTargetSoCMin":
                (
                    log.info(f"Updating BATTERY_LOW to {int(value)}%")
                    if BATTERY_LOW != int(value)
                    else None
                )
                BATTERY_LOW = int(value)
            ...
            case "batteryTargetSoCMax":
                (
                    log.info(f"Updating BATTERY_HIGH to {int(value)}%")
                    if BATTERY_HIGH != int(value)
                    else None
                )
                BATTERY_HIGH = int(value)
